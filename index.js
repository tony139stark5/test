...
ppwelcome = await getBuffer(`https://ziy.herokuapp.com/api/canvas/welcome2?pp=${ppuser}&nama=${nama}&bg=https://i.ibb.co/XjgQzkB/b1be492ada987df650bc831b1631815e.jpg&namagc=${encodeURIComponent(metadata.subject)}&member=${encodeURIComponent(memb)}`)
Tol = await getBuffer(`https://api.lolhuman.xyz/api/base/leave?apikey=ThadzBotZ&img1=${ppuser}&img2=${ppgroup}&background=https://i.ibb.co/8B6Q84n/LTqHsfYS.jpg&username=${nama}&member=${encodeURIComponent(memb)}&groupname=${encodeURIComponent(metadata.subject)}`)
Welcome = `Welcome To ${metadata.subject} ${num.split("@")[0]}\n\n${metadata.desc}`
Goodbay = `${num.split("@")[0]} Leaving To ${metadata.subject}\nGood Bye 👋`
   if (anu.action == 'add') {
...
// Siapa yang cita-citanya pakai resize buat keliatan thumbnailnya , hyzer bilek 
